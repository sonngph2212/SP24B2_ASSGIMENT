...
<div class="container mt-5">
    ...
    <form method="POST" action="/app_war_exploded/hoa-don/store">
        <div class="mb-3">
            <label class="form-label">Khách hàng</label>
            <select class="form-select" name="idKh">
                <c:forEach items="${lstKh}" var="el">
                    <option value="${el.id}">${el.maKhachHang} - ${el.ten}</option>
                </c:forEach>
            </select>
        </div>
        <div class="mb-3">
            ...
            <select class="form-select" name="idNv">
                <c:forEach items="${lstNv}" var="el">
                    <option value="${el.id}">${el.tenDangNhap} - ${el.ten}</option>
                </c:forEach>
            </select>
        </div>
        <div class="mb-3">
            <label class="form-label">Ngày mua hàng</label>
            <input type="date" class="form-control" name="ngayMuaHang" value="${defaultDate}">
        </div>
